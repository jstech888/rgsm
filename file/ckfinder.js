			...
			bz : function M(K) {
				...
				K.on('themeSpace', function P(N) {
					if (N.data.space == 'mainMiddle') {
						...
						if (!g)
							O = u[0] + u[3];
						N.data.html += "<div id='files_view' class='view widget files_thumbnails' aria-live='polite' role='main' tabindex='0' aria-controls='status_view'><h4 style='display:none;' class='message_content'></h4><div class='files_thumbnails fake no_list' role='list'></div>" + O + '</div>';
					}
				});
						...
						exec : function (R) {
							...
							if (S) {
								...
								if (R.config.directDownload)
									...
									T = R.connector.composeUrl('DownloadFile', {
											FileName : S.name
						...
						K.eU({
							...
							nA : {
								...
								onClick : function () {
									...
									S = R.tools.dH();
									if (S)
										R.oW('requestSelectThumbnailAction', {
											file : S
										});
								},
								group : 'file0'
				...
				this.oW('beforeShowFolderFiles', S, function Y(U, V) {
					...
					try {
						...
						this.app.connector.sendCommand('GetFiles', X, function (Z) {
							...
							Q.tools.kR();
							var aa,
							aT = Z.selectNodes('Connector/Files/File');
		...
		CKFinder.addPlugin('zip', {
			uiReady : function (r) {
				...
				CKFinder.dialog.add('unzipDirExists', function (t) {
					var u = t.getSelectedFolder(),
					v = [[t.lang.ManuallyRename, 'manuallyrename']];
