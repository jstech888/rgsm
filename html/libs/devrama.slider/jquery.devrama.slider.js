...
(function($){
	...
	DrSlider.prototype = {
		...
		_transition: function($element, transition, callback){
			...
			var that = this;
			setTimeout(function(){					
				if($('.background:visible').attr("slider-type") == "youtube")
				{
					var videoId = HomeSlider[$('.background:visible').attr("slider-key")].value.videoId;
					$('.background:visible').find("iframe").attr("src",'https://www.youtube.com/embed/'+videoId+'?autoplay=1&loop=1&rel=0&wmode=transparent');
					that.is_pause = true;
					that._hideButtons();
				}		
				if($('.background:visible').attr("slider-type") == "vimeo")
				{
					var videoId = HomeSlider[$('.background:visible').attr("slider-key")].value.videoId;
					$('.background:visible').find("iframe").attr("src",'//player.vimeo.com/video/'+videoId+'?autoplay=1');
					that.is_pause = true;
					that._hideButtons();
				}	
				if($('.background:visible').attr("slider-type") == "youku")
				{
					var videoId = HomeSlider[$('.background:visible').attr("slider-key")].value.videoId;
					$('.background:visible').find("iframe").attr("src",'http://player.youku.com/embed/'+videoId+'?isAutoPlay=true&isShowRelatedVideo=false&embedid=-&showAd=0');
					that.is_pause = true;
					that._hideButtons();
				}
				if($('.background:visible').attr("slider-type") == "tudou")
				{
					var videoId = HomeSlider[$('.background:visible').attr("slider-key")].value.videoId;
					$('.background:visible').find("iframe").attr("src",'http://www.tudou.com/programs/view/html5embed.action?'+videoId);
					that.is_pause = true;
					that._hideButtons();
				}
