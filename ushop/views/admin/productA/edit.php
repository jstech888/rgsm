...
.img-container:hover .btn-tools {
  display: block;
						...
						<div class="panel-body">
								<div class="panel">
									...
									<div class="panel-body">
										...
										<div class="col-sm-12 mb15">
											<div class="form-group">
												<label for="product-parent-id" class="col-xs-4 col-md-3 col-lg-2 control-label" >父層</label>
												<div class="col-xs-7 col-md-8 col-lg-9">
													<select class="form-control" onchange="saveTempProductData('parentId',this);">
												<?php
													$str_selected = ( $data_product['parentId'] == 0 ) ? " selected":"";
												?>
														<option value="0"<?php echo $str_selected;?>>根層</option>
												<?php
													foreach( $list_allCatalog AS $row )
													{
														$str_selected = ( $data_product['parentId'] == $row["id"] ) ? " selected":"";
												?>
														<option value="<?php echo $row["id"];?>" <?php echo $str_selected;?>><?php echo $row["detailKey"];?></option>
												<?php
													}
												?>
													</select>
										...
										<div class="col-sm-12 mb15">
											<div class="form-group">
												<label for="product-qty" class="col-xs-4 col-md-3 col-lg-2 control-label">數量</label>
												<div class="col-xs-7 col-md-8 col-lg-9">
													<input id="product-qty" type="text" class="form-control" onkeyup="saveTempStockData('value',this);" value="<?php echo $data_stock['value']?>">
												</div>
											</div>
											<div class="clearfix"></div>
										</div>
										...
										<div class="panel mb30" id="p6">
											<div class="panel-heading">
												<span class="panel-title">  特殊（商品）運費設定 </span>
											</div>
											<div class="panel-body product-stock-panel-body">
											
													<div class="form-group">
														<label for="product-formart-type" class="col-xs-2 control-label">運費</label>
														<div class="col-xs-7">
															<select id="product-shipping-type" class="form-control" onchange="saveTempProductData('shippingKey',this);">
																<option value="" >不設定（使用全館運費設定）</option>
																<?php
																if( isset($shippingDataAry) && count($shippingDataAry)>0 ){ 
																	foreach ($shippingDataAry as $key => $Aobj) 
																	{
																?>
																	<option value="<?php echo $Aobj['displaykey'];?>" <?php echo ($Aobj['displaykey']== $data_product['shippingKey'])? "selected":"" ?>><?php echo $Aobj['title'];?></option>
																<?php
																	}
																}
																?>
										...
										<div class="tabs-content isoCode-tab-content">
											...
											<div class="col-sm-12 mb15">
												<div class="form-group">
													<label for="product-price-normal" class="col-xs-4 col-md-3 col-lg-2 control-label">加價購</label>
													<div class="col-xs-7 col-md-8 col-lg-9">
														<input id="product-price-normal" type="text" class="form-control" onkeyup="saveTempPriceData('normal',this);saveTempPriceData('value',this);" value="<?php echo $data_price[$Currency]['normal']?>">
													</div>
												</div>
												<div class="clearfix"></div>
											</div>
										...
										<div class="tabs-content main-tab-content">
											...
											<div class="col-sm-12 mb15">
												<div class="form-group">
													<label for="product-name" class="col-xs-4 col-md-3 col-lg-2 control-label">標籤</label>
													<div class="col-xs-7 col-md-8 col-lg-9">
														<input id="product-name" type="text" class="form-control" value="<?php echo $data_detail[$DEFAULTLANG]['tag']?>" onkeyup="saveTempDetailData('tag',this);">
														<span class="help-block mt5"><i class="fa fa-bell"></i> 標籤與標籤間使用半形逗號分隔，例如：<p style="color:red;display:inline-block;">保養,纖體,瘦身</p></span>
													</div>
												</div>
												<div class="clearfix"></div>
											</div>
	...
	Object.size = function(obj) {
		...
		var data_product	= <?php echo json_encode($data_product, JSON_FORCE_OBJECT);?>;
		var data_detail 	= <?php echo json_encode($data_detail, JSON_FORCE_OBJECT);?>;
		var data_price 		= <?php echo json_encode($data_price, JSON_FORCE_OBJECT);?>;
		var data_stock 		= <?php echo json_encode($data_stock, JSON_FORCE_OBJECT);?>;	
		...
		var oftenFormartType = {
			...
			apply: function(inds) {
				...
				formatTypeContent.listContent[inds].often = this.value;
		...
		var formatTypeContent = {
			...
			init : function() {
				...
				if(data_stock[0].formatType != "F")
				{
					var level = Object.size(data_stock[0].formatTypeTitle);//.formatTypeTitle.length;
		...
		var formatTypeTable = {
			...
			fillDB : function() {
				...
				for( var k in data_stock )
				{
					TitleColumn+= "<th class=\"text-center\">"+data_stock[k].formatType+"</th>";
					ContentColumn+= "<td><input class=\"form-control\" value=\""+data_stock[k].value+"\" onkeyup=\"saveTempFormartTypeA('"+this.typeList[k]+"',this.value);\" /></td>";
			...
			refresh : function() {
				...
				{
					TitleColumn+= "<th class=\"text-center\">"+this.typeList[k]+"</th>";
					ContentColumn+= "<td><input class=\"form-control\" value=\"0\" onkeyup=\"saveTempFormartTypeA('"+this.typeList[k]+"',this.value);\" /></td>";
		...
		function selectMediaStackSlider()
		{
			MediaStack.popup({
				...
				selectActionFunction : function(e){
					...
					$(".product-image-slider-container").append(newImg);
					var lang = $(".main-langCode li.active a").attr("data-langCode");
					if ( typeof(data_detail[lang].value) == "undefined" || data_detail[lang].value == null  ){
						data_detail[lang].value = new Object(); 
					}
					( typeof(data_detail[lang].value.image) == "undefined" ) ? data_detail[lang].value.image = [] : "";
					data_detail[lang].value.image[Object.size(data_detail[lang].value.image)] = file;
				}
			});
		}
