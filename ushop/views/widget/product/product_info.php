...
if ( is_array( $product [ "stock" ] ) )
                            {
                              ...
                              <div class="form-group col-md-12">
                              	<div class="quantity">
									<div class="name"><label><?php echo $stockTitleStr; ?>： </label></div>
									<div class="total">
										<select class="formatType-select form-control" data-pid="<?php echo $product["PID"];?>" style="width:100px;">
											<option value=""><?php echo $objLang["product"]["selectWording"];?></option>
			                                <?php foreach( $product["stock"] AS $row ) { ?>
			                                  <option value="<?php echo $row["id"];?>"> <?php echo $row["formatType"];?></option>
			                                <?php } ?>
                              			</select>
										<script>
										var formatTypeStock = [];
										<?php foreach ( $product["stock"] as $spec ) { ?>
			...
			<div class="row">
                          ...
                          <div class="form-group col-md-12">
					<div class="quantity">
						<div class="name"><label><?php echo $objLang["product"]["qty"]; ?>： </label></div>
						<div class="total"> 
							<select class="quantity btn-selectQty-to-add form-control" data-pid="<?php echo $product["PID"];?>">
								<?php 
									if(is_array($product["stock"])){ 
								?>
								...
								<?php 
										...
										$maxNum = $product["stock"]>12? 12:$product["stock"];
										for($i=1;$i<=$maxNum;$i++){	
								?>
							...
							<?php /*<input type="number" class="quantity btn-selectQty-to-add" value="1" step="1" min="1" max="20" data-pid="<?php echo $product["PID"];?>">*/?>
						</div>
		...
		<div id="productPromTabs" class="tab-block mb25 mt25">
			...
			<div class="tab-content">
				<div id="productProm1" class="tab-pane active">
					<?php
						echo isset($product['value']->description2) ? $product['value']->description2 : "";
				...
				<div id="productProm2" class="tab-pane">
					<?php 
						echo isset($product['value']->description3) ? $product['value']->description3 : "";
