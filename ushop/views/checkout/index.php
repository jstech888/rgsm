                    ...
                    <div class="lb-value" <?php echo ($ShoppingPointMaxRate!=0 && $ShoppingPointMaxRate!='')? 'style="width:initial;" ':'';?>>
                      <div class="input-group">
                        ...
                        <span class="input-group-btn">
                          <button class="btn btn-secondary" type="button" onclick="checkoutFlow.go(3);" style="margin-left: 2px;">
...
{ 
  ...
  if(nowStep==3){
    // 手續費 / 運費
    objFeeFare = readFeeFare();
// alert('region='+$('input[name="region"]:checked').val());
    // if($('input[name="region"]:checked').val()!=2){
// alert("isFreeFareRuled="+isFreeFareRuled);
    if(isFreeFareRuled){
	    ( ( shoppincart_twTotal - checkoutData.shoppingPoint ) > FreeFare )? objFeeFare.fare = 0:''; 
    }
// alert('objFeeFare.fare='+objFeeFare.fare);
    $("#trans-fare").text(objFeeFare.fare);
	$("#trans-fee").text(objFeeFare.fee);	
...
function readFeeFare()
{
  ...
  isFreeFareRuled = (sRegion=="inIsland")? true : false;
...
// alert('checkoutData.cashFlow='+checkoutData.cashFlow);  return;
  ...
  checkoutData.shipFlowData.region    = $('input[name="region"]:checked').val();
