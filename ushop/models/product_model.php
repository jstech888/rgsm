...
class Product_model extends My_Model {
		...
		$sql = "SELECT *,
                  ...
                  `product_price`.`value` AS 'sell',
                   ( CASE
		            ...
		            (select warnValue FROM `product_stock` WHERE `product_stock`.`stockKey` = pa.`stockKey` LIMIT 1) warnValue
                FROM `product` pa
	...
	public function listAllTags($langCode = false)
	{
		$lang = ($langCode == false)?$this->currentLang:$langCode;
		
		$result = array();
		
		$sql = "SELECT DISTINCT `tag` FROM `product_detail` WHERE `langCode` = ? AND `tag` != '';";
		$arrRaw = $this->db->query($sql,array($lang))->result_array();	
		foreach( $arrRaw AS &$row ) {
			$arrTags = explode(",",$row["tag"]);
			foreach( $arrTags AS $tag )
			{
				in_array($tag,$result)?"":$result[]=$tag;
			}
		}
		return $result;
	}
	...
	public function findTag($tag)
	{
		$sqlsearchKeyWords = "";
		$keyAry = array() ;
		if(count(explode(",",$tag) ) > 1 || strlen($tag) > 0 ){
			$sqlsearchKeyWords = " (" ;
			$keyAry = explode(",",$tag) ; 
			$tmpcnt = 0 ;
			...
			foreach ($keyAry as $key => $value) {
				$tmpcnt++ ;
				$sqlsearchKeyWords .= " `product_detail`.`tag` like '%".trim($keyAry[$key])."%' " ;
				if ( count($keyAry) > $tmpcnt ) $sqlsearchKeyWords .= " OR " ;
			}
			$sqlsearchKeyWords .= " )  " ;
		}
	...
	public function findDetailStock($id,$formatType="F")
	{
		$this->db->select("*");
		$this->db->from("product_detail");
		$this->db->join('product_stock',"product_stock.stockKey=product_detail.detailKey");
		$this->db->where(array("product_detail.detailKey"=>$id));
		if($formatType!="F")		
			$this->db->where(array("product_stock.id"=>$formatType));
		else
			$this->db->where(array("product_stock.formatType"=>$formatType));
		$this->db->where(array("product_detail.langCode"=>$this->currentLang));
		$result = $this->db->get()->result_array();
		$new_result = array();
		foreach( $result AS $row )
		{
			$new_row = $row;
			$new_row["formatTypeTitle"] = json_decode($row["formatTypeTitle"],true);
			$new_result[] = $new_row;
		}
		$this->last_result = $new_result[0];
		return $new_result[0];
		// return $result;
	}
	...
	{
		$sample = array("parentId","detailKey","priceKey","stockKey", "itemNo", "brand", "updateTime","touch","flag","flagVar","status","type");
	...
	public function reduceStock( $delta, $stockKey, $formatType = "F", $isKey = true )
	{
		if( $isKey === false )
		{
		...
		$sql.= $formatType=='F'? "`formatType` = '$formatType'":"`id` = '$formatType'";
// error_log(date("YmdHis").":\r\n".print_r($sql,true)."\r\n\r\n",3,"uploads/log_productModel_reduceStock_sql.log");
		$this->db->query($sql);
		return $this->db->affected_rows();
	}
	...
	public function find( $id, $lang = false, $currency = false, $flag = false )
	{	
		if( empty($id) )
		{
			return array();
		...
		$flagVar = ( $flag === false )?"":" AND `product`.`flag` = 1";
		$cPrice = "
			,CASE 
	...
	private function recursiveFindParentById($id, &$new_res)
	{
		...
		$sql = "SELECT * FROM `product` AS c WHERE c.id IN ('$id');";
		// $sql = "SELECT * FROM `product` AS c WHERE c.id = $id;";
		$result = $this->db->query($sql)->result_array();
		if( count($result) > 0 )
		{
	...
	private function findChildCatelogById($id,&$new_result)
	{
		if( sizeof($id) > 0 )
		{
			$sql = "SELECT *
					  FROM `product`
					 WHERE parentId = $id AND status = '0' ORDER BY sortkey ;";
			$result = $this->db->query($sql)->result_array();
			///echo $this->db->last_query()."<br/>";
			if( count($result) > 0 )
			{
	...
	public function checkWarnValue($stockKey, $formatType){
	    ...
	    $sql = "select * from `product_stock` WHERE `stockKey` = '$stockKey' AND ";
	    $sql.= $formatType=='F'? "`formatType` = '$formatType'":"`id` = '$formatType'";
error_log(date("YmdHis").":\r\n".print_r($sql,true)."\r\n\r\n",3,"uploads/log_productModel_checkWarnValue_sql.log");					    
	    $stockRow = $this->db->query($sql)->row_array();
error_log(date("YmdHis").":\r\n".print_r($stockRow,true)."\r\n\r\n",3,"uploads/log_productModel_checkWarnValue_stockRow.log");				
