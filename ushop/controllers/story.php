...
class Story extends Web_Controller {
	...
	{		
		...
		$this->data["method"] = "videos";
		$this->data["titleName"] = "影片介紹" ;
		// $this->data["titleName"] = $this->data['objLang']["function_bar"]["media"] ;
		
		$this->data["listStory"] = $this->Article_model->findAll("movies");
	...
	public function detail($id = false)
	{
		if( $id !== false )
		{
			...
			$this->data["titleName"] = "影片介紹" ;
			// $this->data["titleName"] = $this->data['objLang']["function_bar"]["media"] ;
			$arr_article = $this->Article_model->admin_find("movies", $id);
			if( count($arr_article) > 0 )
			{
				$this->data["og"] = array(
					"url" => base_url("/videos/detail/".$id.'/'),
					"type" => "article",
