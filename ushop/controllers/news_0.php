...
class News extends Web_Controller {
	...
	{		
		...
		else
		{
			...
			$this->data["HomeSlider"] = $this->mWidget->find("blogSlider");
			$this->data["WHSlidertEffect"] = $this->mOption->readVal("BlogSlider-Effect");
								
			$this->load->model("Article_model");	
			
			$this->data["allClassTopFive"] = $this->Article_model->allClassTopFive();
			
			$this->data["hotStory"]  = $this->Article_model->classHome( false, 0, 5 );
			$this->data["TotalRaty"] = $this->mOption->readString("TotalRaty");
				
			$allBloger = $this->Article_model->allBloger();		
			$this->data["classBloger"] = $allBloger;
			
			$this->load->view('inc/head',$this->data);
			$this->load->view('blog/homeIndex',$this->data);
			$this->load->view('inc/footer',$this->data);
		}
	}
	...
	public function detail($id = false)
	{
		if( $id !== false )
		{
			...
			$arr_article = $this->Article_model->admin_find("blog", $id);
			if( count($arr_article) > 0 )
			{
				
				$listMessage = $this->Article_model->findResponseMessage($id);
				...
				//版主最新文章
				$authorLastArticle = $this->Article_model->admin_find("blog", false, $arr_article[0]["author"]);
		...
		if( array_key_exists( 'blogTouch', $this->webVar ) )
		{
			...
			else
			{
				...
				foreach( $this->webVar['blogTouch'] AS &$touch )
				{
					...
					if( time() - $limit > 0 )
					{
						$result = $this->Article_model->touchArticle( $touch["AID"], $touch["ClassKey"], $touch["blogerId"] );
	...
	public function reply()
	{
		...
		{		
			$this->load->model("Article_model");	
			$this->Article_model->replyMessage( $_POST["aid"], $this->self["id"], $_POST["msg"]);
		}
	}
	...
	public function moderators($id = false)
	{
		if( $id !== false )
		{
			...
			if( in_array($user[0]["group_id"], $arrMod) && $blog["id"] != -1 )
			{
				...
				//版主最新文章
				$authorLastArticle = $this->Article_model->admin_find("blog", false, $id, "markDate" );
				...
				//版主熱門文章
				$authorHotArticle = $this->Article_model->admin_find("blog", false, $id, "touch" );
