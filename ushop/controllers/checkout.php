...
class Checkout extends Web_Controller {
	...
	public function index()
	{
		...
		$DateLimitCheckoutDiscount = $this->data["DateLimitCheckoutDiscount"];
       	...
       	if($DateLimitCheckoutDiscount["inTerm"] === true) {
            if ( floatval($cart["total_main_prodcut"]) >= floatval($DateLimitCheckoutDiscount["LimitAmount"]) )
            {
            	$discount = $cart["total_main_prodcut"] * round(( floatval( 1 - $DateLimitCheckoutDiscount["Rate"] ) ),2);
            }
        }
        $cart['total_main_prodcut_discount'] = $discount;

        foreach ($cart['calProduct'] as $key => $price_promotion) {
        	if($key>0){
	        	if($price_promotion!=0){
	        		$final_total_price = $final_total_price + $price_promotion;
	        	}
	        	else{
	        		$final_total_price = $final_total_price + $cart['total_promotion'][$key];	
	        	}
	        }
        }
        $final_total_price += $cart['total_main_prodcut'] - $discount;
        $cart['total'] = round($final_total_price);
  
		$this->data['ucart'] = $cart;

		if($this->data['ucart']['total'] == 0 )
				redirect('/shoppingcart', 'location', 301);

		$this->load->model("hall_model","hallM");
		$this->data['allEffectHall'] = $this->hallM->findEffectHall();
		...
		$this->data["ShoppingPointMaxRate"] = $this->mOption->readVal("ShoppingPointMaxRate");
	...
	//判斷是否符合免運費規則
	private function isFreeFareRuled($objAddr)
	{
		$outsea =  array("澎湖縣","金門縣","連江縣","南海諸島","綠島鄉","蘭嶼鄉");
		...
		$result = ($sRegion=="inIsland")? true : false;
...
// error_log(print_r($_REQUEST["cashFlowData"],true)."\r\n\r\n",3,"uploads/log_checkout_REQUEST[cashFlowData].log");
// error_log(print_r($_REQUEST,true)."\r\n\r\n",3,"uploads/log_checkout_REQUEST.log");
			$tran_main['cashFlow'] 		= $_REQUEST['cashFlow'];
			$tran_main['cashData'] 		= json_encode($cashFlowData);
// error_log(print_r($tran_main["cashData"],true)."\r\n\r\n",3,"uploads/log_checkout_tran_main[cashData].log");
			...
			$invoiceData 				= $_REQUEST['invoiceData'];
			$tran_main['invoiceData'] 	= json_encode($invoiceData);
			...
			$isFreeFareRuled = $this->isFreeFareRuled($shipFlowData);
			// if($shipFlowData['region']!=2){
			if($isFreeFareRuled){
				...
				$tran_main['transFare'] = $listFeeFare['fare'];
			}
			
			//訂單毛額/總金額
			$tran_main['total'] 		= $ucart['twTotal'];
			$tran_main['clearTotal'] 	= floatval($ucart['twTotal']) - floatval($shoppingPoint) - floatval($coupon["amount"]) + floatval($tran_main['transFee']) + floatval($tran_main['transFare']);
...
// error_log(date("YmdHis").":\r\n".print_r($dataStock,true)."\r\n\r\n",3,"uploads/log_checkout_dataStock.log");
				// foreach ($dataStock as $key => $stockData) {
				// 	if($stockData['value']<$stockData['warnValue']){
				// 		$prodct_detail = $this->Product_model->findDetail($mainKey);
				// 		$mail_content.="商品: <a href='' target='_blank'>".$prodct_detail['name']."</a>"
				// 		if($stockData['formatType']!="F")
				// 			$mail_content.=", 規格: ".$stockData['formatType']." 之庫存已低於安全庫存量，請確認!<br>";
				// 	}
				// }
				$realStock 		= $this->findRealStock($dataStock, $row['formatType']);
// error_log(date("YmdHis").":\r\n".print_r($realStock,true)."\r\n\r\n",3,"uploads/log_checkout_realStock.log");				
				$qty = intval( $row['qty']) ;
				if( $realStock < $qty ){
					$prodStockFlag = false ;
					break;
...
// error_log(date("YmdHis").":\r\n".print_r($arrProd_rs,true)."\r\n\r\n",3,"uploads/log_checkout_arrProd_rs.log");
...
// error_log(date("YmdHis").":\r\n ordTotalFromDB=".$ordTotalFromDB.", ucart['twTotal']=".$ucart['twTotal'],3,"uploads/log_checkout_ordTotalCheckFlag.log");
			...
			$checkWarn = array();
// error_log(date("YmdHis").":\r\n".print_r($ucart,true)."\r\n\r\n",3,"uploads/log_checkout_ucart.log");
// error_log(date("YmdHis").":\r\nprodStockFlag=".$prodStockFlag.", ordTotalCheckFlag=".$ordTotalCheckFlag."\r\n",3,"uploads/log_checkout_flagchecks.log");
...
error_log(date("YmdHis").":\r\n".print_r($checkWarn,true)."\r\n\r\n",3,"uploads/log_checkout_checkWarn.log");				
// error_log(print_r($tran_main,true)."\r\n\r\n",3,"uploads/log_checkout_tran_main.log");				
				...
				$mail_name	  = ($isLogin) ? $this->data['self']['nickname'] : $nonUserInfo["fullname"];
...
// error_log(print_r($this->data['self'],true),3,"uploads/log_controller_checkout_this->data[self].log");
// error_log(print_r($nonUserInfo,true),3,"uploads/log_controller_checkout_nonUserInfo.log");
// error_log(print_r($OrderSuccessMail,true),3,"uploads/log_controller_checkout_OrderSuccessMail.log");
// error_log("mail_name=".$mail_name.", result=".$result."\r\n\r\n",3,"uploads/log_controller_checkout_OrderSuccessMail2.log");
				...
				if (@$checkWarn){
				    ...
				    foreach ($checkWarn as $k => $v){
    				    ...
    				    $detail = $this->Product_model->findDetailStock($v['detailKey'], $v['formatType']);
// error_log(print_r($detail,true),3,"uploads/log_controller_checkout_detail.log");
// error_log(print_r($prodct_detail,true),3,"uploads/log_controller_checkout_prodct_detail.log");
    				    // $prodct_stock = $this->Product_model->findStock($v['detailKey']);
// error_log(print_r($prodct_stock,true),3,"uploads/log_controller_checkout_prodct_stock.log");
						$message.="商品: <a href='".base_url("/admin/product/edit?id=".$v["detailKey"])."' target='_blank'>".$detail['name']."</a>";
						if($v['formatType']!="F") $message.=", 規格: ".$detail['formatType'];
						$message.=" 之實際庫存量(數量:".$detail['value'].") 已低於安全庫存量(數量:".$detail['warnValue'].") ，請進快確認處理! \n";
    				    // $message .= "{$detail['name']} {$v['formatType']} 低庫存警告數量設定 : {$detail['warnValue']}, 實際庫存 : {$detail['value']}    \n";
				    }

				    $title = "商品低庫存警告信(".date('Y')." 年 ". date('m') ." 月 ". date('d') ." 日 " .date('H'). "時". date('i') ."分)";

				    sendMailBySystemMail($AdminMailStr, $AdminMail["account"], $title/*'庫存警告信'*/, $message);
				}
