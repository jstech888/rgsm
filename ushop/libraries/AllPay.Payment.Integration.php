...
class AllInOne {
    ...
    function CheckOutString($paymentButton, $target = "_self") {
        ...
        if (sizeof($arErrors) == 0) {
			...
			$szHtml = "<html><head><meta http-equiv=\"Content-Language\" content=\"zh-hant\" /><meta http-equiv=\"Content-Type\" content=\"text/html\" charset=\"utf-8\" /></head><body onload=\"document.getElementById('form1').submit();\"><form id=\"form1\" name=\"form1\" target=\"".$target."\" action=\"".$this->ServiceURL."\" method=\"POST\">";
			
			foreach( $arParameters AS $name => &$value )
			{
				//( in_array($name,$urlencode) )?urlencode($value):"";
				$szHtml.= "<input type=\"hidden\" name=\"{$name}\" value=\"{$value}\" />";
			}
            $szHtml.= '<input type="hidden" name="CheckMacValue" value="' . $szCheckMacValue . '" />';
			$szHtml.= "</form></body></html>";
			/*
